...
from .models import Tour, OrderTour, Customer, Hotel, OrderHotel, Checkoutenquiry, VendorRequest, Payment ,Hotelcheckout
...
def save_payment(request):
    if request.method == "POST":
        ...
        pt = Payment(user=user,phone=phone)
        pt.save()
...
def search_tours(request):
    ...
    if request.method == 'POST':
        ...
        tourTraveller = request.POST["tourTravellers"]
...
def hotelcheckout(request):
    return render(request, 'blog/hotelcheckout.html')
...
def save_hotelcheckout(request):
    if request.method == "POST":
        ...
        mm = Checkoutenquiry(first_name=first_name,second_name=second_name,email=email,address=address,phone=phone, tour_date=tour_date,tour_travellers=tour_travellers)
        mm.save()
...
def orderHotel(request, id):
    ...
    if created:
        messages.info(request, "This item is added to your cart!")
        return redirect("blog:hotelcheckout")
    else:
        order_item.save()
        messages.info(request, "This item is added to your cart.")
        return redirect("blog:hotelcheckout")
